...
Safe: "V2 improves deterministic skill routing on the active acceptance suite and remains strong on the post-tuning challenge, fresh-probe regression, and frozen-holdout regression suites, while the current clean holdout exposes unresolved V2 generalization gaps."

Safe: "V2 has 0.0% forbidden primary rate across the current active, challenge, fresh-probe regression, and frozen holdout regression suites, while improving output quality versus both compared baselines. The clean holdout is the counterexample and must be cited separately."
...
- Clean holdout V2 suite: `benchmarks/skill-routing-clean-holdout-v2.json`, generated into `docs/SKILL-USE-CLEAN-HOLDOUT-V2.md`. This is the current untouched baseline captured after commit `3cd6e51`; it is non-gating and should not drive tuning until cases are deliberately promoted or relabeled.

The current challenge suite started as a frozen pilot, then its misses were used to improve broad routing anchors and specialist concept membership. From this commit forward it is a fixed post-tuning challenge suite, not pristine untouched holdout evidence. The clean holdout V2 suite is the current clean-split evidence and currently reports failures.
...
Clean holdout V2 baseline generated by `npm run benchmark:skills:clean` on June 27, 2026:
...
These snapshots are strong evidence for the active, challenge, fresh-probe regression, frozen-holdout regression, and clean-holdout baseline suites, not proof of universal routing correctness. The clean holdout currently argues against a broad V2 generalization claim.
